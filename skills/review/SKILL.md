---
description: Full review of the current issue's implementation -- run tests, check standards, demo user tests.
---

Perform a full review of the current issue's implementation.

1. Run `make test` and paste the output (includes lint) -- unless it has just been run and no changes have been made since then, in which case state this and skip.
2. Verify: zero errors, no new warnings. These are hard blocks -- no exceptions.
3. List each coding standard section you checked against, by name and section number.
4. For each UT: launch the application/tool, show me what's on screen, and ask "Does this pass UT-{issue}.{n}?" as a yes/no question. Never give me instructions to run something myself.
5. Update the AC table **in place**. Update automated test statuses. Leave UTs as pending until I answer.
6. Update project documentation as appropriate.
7. Commit with message `Implement #[n]: [short description]` and push.
8. Add a comment to the issue: implementation details, testing instructions, commit link.
9. Summarize: restate the issue, list all actions taken during this implementation, and present the results.

**End with:** `READY FOR REVIEW - issue #NNN` and the issue link. **STOP.**
